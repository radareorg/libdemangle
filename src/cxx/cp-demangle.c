...
cplus_demangle_fill_ctor (struct demangle_component *p,
                          ...
                          struct demangle_component *name)
{
	if (!p || !name || (int)kind < gnu_v3_complete_object_ctor || (int)kind > gnu_v3_object_ctor_group) {
		return 0;
	}
...
cplus_demangle_fill_dtor (struct demangle_component *p,
                          ...
                          struct demangle_component *name)
{
	if (!p || !name || (int)kind < gnu_v3_deleting_dtor || (int)kind > gnu_v3_object_dtor_group) {
		return 0;
	}
  ...
  if (peek == 'G' || peek == 'T') {
	  ...
	  if (dc != NULL && top_level && (di->options & DMGL_PARAMS) == 0) {
	     ...
	     v2 demangler without DMGL_PARAMS.  */
		  while (dc->type == DEMANGLE_COMPONENT_RESTRICT_THIS || dc->type == DEMANGLE_COMPONENT_VOLATILE_THIS || dc->type == DEMANGLE_COMPONENT_CONST_THIS || dc->type == DEMANGLE_COMPONENT_REFERENCE_THIS || dc->type == DEMANGLE_COMPONENT_RVALUE_REFERENCE_THIS) {
			  dc = d_left (dc);
		  }
	     ...
	     which is local to a function.  */
		  if (dc->type == DEMANGLE_COMPONENT_LOCAL_NAME) {
			  ...
			  dcr = d_right (dc);
			  while (dcr->type == DEMANGLE_COMPONENT_RESTRICT_THIS || dcr->type == DEMANGLE_COMPONENT_VOLATILE_THIS || dcr->type == DEMANGLE_COMPONENT_CONST_THIS || dcr->type == DEMANGLE_COMPONENT_REFERENCE_THIS || dcr->type == DEMANGLE_COMPONENT_RVALUE_REFERENCE_THIS) {
				  dcr = d_left (dcr);
			  }
...
d_parmlist (struct d_info *di)
{
  ...
  /* There should be at least one parameter type besides the optional
    ...
    if (!d_right (tl) && d_left (tl)->type == DEMANGLE_COMPONENT_BUILTIN_TYPE && d_left (tl)->u.s_builtin.type->print == D_PRINT_VOID) {
	...
	while (typed_name != NULL)
	  {
	    ...
	    if (typed_name->type != DEMANGLE_COMPONENT_RESTRICT_THIS && typed_name->type != DEMANGLE_COMPONENT_VOLATILE_THIS && typed_name->type != DEMANGLE_COMPONENT_CONST_THIS && typed_name->type != DEMANGLE_COMPONENT_RVALUE_REFERENCE_THIS && typed_name->type != DEMANGLE_COMPONENT_REFERENCE_THIS) {
		    break;
	    }
